...
def main():
    ...
    parser.add_argument('--sizes', nargs='+', type=int, required=False,
    ...
    mlx_variants = ['searchsorted_integrated', 'searchsorted_linear', 'searchsorted_binary']
    ...
    for size in args.sizes:
        ...
        for variant in mlx_variants:
            print(f"\n-- MLX ({variant}) --")
            run_command(bench_mlx_cmd + [variant, '--size', str(size)] + (['--cpu'] if args.cpu else []))

        # Run PyTorch (single implementation)
        print('\n-- PyTorch (torch.searchsorted) --')
        run_command(bench_torch_cmd + [torch_variant, '--size', str(size)] + (['--cpu'] if args.cpu else []))
